...
class Vista_carrera(QWidget):
    ...
    def aniadir_competidor(self):
        ...
        if dialogo.resultado == 1:
            # Intentar convertir la probabilidad a float
            try:
                probabilidad = float(dialogo.texto_probabilidad.text())
            except ValueError:
                # Mostrar un mensaje de error si la probabilidad no es un número flotante válido
                QMessageBox.warning(self, "Error de Validación", "La probabilidad debe ser un número válido.")
                return  # No continuar con el guardado

            # Si todo está bien, añadir el competidor
